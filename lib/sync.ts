...
import { AgentMessage, AgentSession, Folder, Word, WordFolder } from "./types";
...
export async function syncData(userId: string) {
    ...
    try {
        ...
        await syncTable<WordFolder>("wordFolders", db.wordFolders, userId, "word_folders", true);

        // 4. Agent Sessions
        await syncTable<AgentSession>("agentSessions", db.agentSessions, userId, "agent_sessions");

        // 5. Agent Messages
        await syncTable<AgentMessage>("agentMessages", db.agentMessages, userId, "agent_messages");
...
async function syncTable<T>(
    ...
    if (localData.length > 0) {
        ...
        const { error: pushError } = await supabase
            ...
            .upsert(recordsToPush, { onConflict: isComposite ? 'word_id, folder_id' : 'id' });

        if (pushError) console.error(`Error pushing ${supabaseTableName}:`, pushError);
    }
    ...
    if (remoteData && remoteData.length > 0) {
        const recordsToPull = remoteData.map((item: any) => transformFromSupabase(item, dexieTableName));
